...
def populateData():
  ...
  if isDataPopulated==False: #makes sure table hasn't been populated to save calls to Yelp API
    ...
    for res in yelp_data["businesses"]:
      ...
      categories_alias_list = [] #creates a list that tracks the alias names of the categories attributed to each restaurant
      categories_title_list = [] #creates a list that tracks the title names of the categories attributed to each restaurant
      ...
      for category_index in range(len(res_categories_list)): #loop through the list of category dictionaries provided for each resturaunt
        ...
        categories_title_list.append(current_category["title"]) #add that title to the list of titles we know this resturaunt has
          
      categories_alias_string = ', '.join(categories_alias_list)
      categories_title_string = ', '.join(categories_title_list)
      ...
      res_id=res["id"] #sets res_id to the "id" key that was found in the yelp_data item
      res_name=res["name"]
      ...
      res_category_title = categories_title_string
      #deserializes all the yelp_data
      # Come back and add the categories stuff
      restaurant = YelpRestaurantSerializer(data={"business_id":res_id,"name":res_name,"yelp_rating":res_rating,"phone_number":res_phone,
                                                      ...
                                                      "yelp_url":res_yelpurl,"categories_alias":res_category_alias,"categories_title":res_category_title},context={'request': None})

      # FIRST IDEA WE HAD FOR THE CATEGORIES (NOT USED FOR THE FILTERING)
      ...
      if restaurant.is_valid(): #checks to see if the data is able to be added to the model
        ...
        restaurant.save()#saves the restaurant to the database
      else:
        print(restaurant)#prints the serializer info
        print(restaurant.errors)#prints the errors
...
def deleteData():
  YelpRestaurant.objects.all().delete() #deletes all data in YelpRestaurant table
  YelpCategories.objects.all().delete() #deletes all data in YelpCategories table
  SustainabilityRating.objects.all().delete() #just for testing
  global isDataPopulated
  isDataPopulated=False #sets isDataPopulated back to false since it was deleted
